...
class SSO extends SmartContract {
    ...
    authorize(authNProof: AuthProof, scope: Scope) {
        ...
        let included = Field(0);
        authNProof.publicInput.scopes.forEach((publicScope) => {
            included = Circuit.if(publicScope.equals(hashedScope), included.add(Field(1)), included);
        });
        included.assertGte(1);
    }
}
